...
BASE_FRICTION_FRONT = 0.2
BASE_FRICTION_BACK = 1
...
def verify():
	file_list = glob.glob('friction_noise/experiment_logs/*.pickle')
	...
	with open('friction_noise/thick_feet_baseline.pickle', 'rb') as f:
		params = pickle.load(f)

	_, res = verify_experiment(experiment, params=params)
	results[0] = res

	with open('friction_noise/verify_results.pickle', 'wb') as f:
...
def plots(abstract=False, presentation=False):
	...
	with open('friction_noise/verify_results.pickle', 'rb') as f:
	...
	if abstract:
		plt.savefig('/Users/Siebe/Dropbox/Thesis/writing/abstract/figures/friction_noise.png', dpi=300)
	elif presentation:
		ax.set_ylim(0, 0.35)
		plt.title('Adding noise on friction')
		plt.tight_layout()
		plt.savefig('/Users/Siebe/Dropbox/Thesis/presentation_plots/friction_noise.png', dpi=300)
	else:
		plt.savefig('friction_noise/speed.pgf')
...
def extract_params():
	...
	for filename in file_list:
		with open(filename, 'rb') as f:
			...
			with open('friction_noise/params/friction_noise_' + str(std_dev_percent) + '.pickle', 'wb') as f2:
