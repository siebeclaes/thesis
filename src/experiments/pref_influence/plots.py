#
# This script creates the plots to evaluate the influence of the P_ref parameter
# on the optimization process. The results are discussed in section 5.1 of my thesis.
#
...
pref_label = 'Reference power \$P_{ref}\$ (W)'
cot_label = 'COT (power / speed)'
...
def is_running_gait(params):
	...
	if hind_offset > np.pi*3/2 or hind_offset < np.pi/2:
		return True
	else:
		return False

def hind_offset_vs_pref(simulations, folder):
	p_refs, offsets = [], []
	...
	plt.scatter(p_refs, offsets, color='#5DA5DA')
	plt.plot([0, 4.5], [np.pi/2, np.pi/2], color='#F15854')
	plt.plot([0, 4.5], [np.pi/2*3, np.pi/2*3], color='#F15854')
	plt.xlabel(pref_label)
	plt.ylabel('Phase offset (radians)')
	plt.yticks([0, np.pi/2, np.pi, np.pi/2*3, np.pi*2],
	...
	plt.savefig(os.path.join(folder, 'hind_offset_vs_pref.pgf'))
...
def speed_vs_cot(simulations, folder):
	...
	plt.legend()
	plt.tight_layout()
	plt.savefig(os.path.join(folder, "speed_vs_cot.pgf"))
...
def speed_vs_power(simulations, folder):
	...
	plt.legend()
	plt.tight_layout()
	plt.savefig(os.path.join(folder, "speed_vs_power.pgf"))
	# plt.savefig("/Users/Siebe/Dropbox/Thesis/writing/figures/speed_vs_cot.pgf")
	# plt.show()
...
def cot_vs_pref(simulations, folder):
	...
	plt.legend()
	plt.tight_layout()
	plt.savefig(os.path.join(folder, "cot_vs_pref.pgf"))
...
def speed_vs_pref(simulations, folder):
	...
	plt.legend()
	plt.tight_layout()
	plt.savefig(os.path.join(folder, 'speed_vs_pref.pgf'))
...
def power_vs_pref(simulations, folder):
	...
	plt.legend()
	plt.tight_layout()
	plt.savefig(os.path.join(folder, 'power_vs_pref.pgf'))
...
def main():
	folders = ['correct_inertia', 'correct_uniform', 'correct_uniform_battery', 'heavy', 'heavy_battery']
	# folders = ['correct_inertia', 'correct_uniform', 'correct_uniform_battery']
	# folders = ['correct_uniform_battery']
	...
	for folder in folders:
		...
		with open(simulation_file, 'rb') as f:
			...
			hind_offset_vs_pref(simulations, folder)
