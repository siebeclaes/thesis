...
# 
# This is the same as the plots.py script but combines the plots of the 
# different morphologies in a single figure.
#
...
def speed_vs_cot(simulations, folder):
	...
	plt.xlabel('Cost of transportation (power / speed)')
...
def cot_vs_pref(simulations, folder):
	...
	plt.ylabel('Cost of transportation (power / speed)')
...
def main():
	# plot_names = ('speed_vs_pref', 'cot_vs_pref', 'power_vs_pref', 'speed_vs_cot', 'speed_vs_power')
	plot_names = ('speed_vs_cot', 'speed_vs_pref')
	# plot_functions = (speed_vs_pref, cot_vs_pref, power_vs_pref, speed_vs_cot, speed_vs_power)
	plot_functions = (speed_vs_cot, speed_vs_pref)
	folders = ['correct_inertia', 'correct_uniform', 'correct_uniform_battery', 'heavy']
	get_axis_limits(folders)

	for plot_name, plot_function in zip(plot_names, plot_functions):
		...
		for folder, sp in zip(folders, subplots):
			...
			with open(simulation_file, 'rb') as f:
				simulations = pickle.load(f)
				plot_function(simulations, folder)

		plt.savefig(os.path.join('combined', plot_name + '.pgf'))
