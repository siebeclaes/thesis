...
EXPERIMENT_TAG = 'vary_body_length_1'
NUM_OPTIMIZATION_STEPS = 150
E_0 = 30
NUM_VARIATIONS = 15

def perform_experiment(body_length_variance):

	print('Experiment "' + EXPERIMENT_TAG + '": ' + str(body_length_variance))
	...
	e = Experiment(model_config, False, initial, lb, ub, 0.5, NUM_OPTIMIZATION_STEPS, E_0, variation_params=variation_params_spring, num_variations=NUM_VARIATIONS, perturbation_params=None, experiment_tag=EXPERIMENT_TAG, experiment_tag_index=body_length_variance)
	e.run()
...
def view_results():
	...
	axarr[0].set_xlabel('Body length standard deviation')
