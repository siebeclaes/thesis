...
def get_std_dev_percent(ex):
	...
	std_dev_percent = int(np.ceil(float(std_dev) / BASE_STIFFNESS * 100))
...
def verify():
	file_list = glob.glob('spring_noise/experiment_logs/*.pickle')
	...
	with open('spring_noise/thick_feet_baseline.pickle', 'rb') as f:
		params = pickle.load(f)

	_, res = verify_experiment(experiment, params=params)
	results[0] = res

	with open('spring_noise/verify_results.pickle', 'wb') as f:
...
def plots(abstract=False, presentation=False):
	...
	with open('spring_noise/verify_results.pickle', 'rb') as f:
	...
	for train_percent, plot_data in plots.items():
		...
		if 'test_percentages' not in data:
			data['test_percentages'] = percentages

		data['train_' + str(train_percent) + '_percent'] = avg_speeds
	...
	if abstract:
		plt.savefig('/Users/Siebe/Dropbox/Thesis/writing/abstract/figures/spring_noise.png', dpi=300)
	elif presentation:
		ax.set_ylim(0, 0.35)
		plt.title('Adding noise on spring stiffness')
		plt.tight_layout()
		plt.savefig('/Users/Siebe/Dropbox/Thesis/presentation_plots/spring_noise.png', dpi=300)
	else:
		plt.savefig('spring_noise/speed.pgf')
...
def extract_params():
	...
	for filename in file_list:
		with open(filename, 'rb') as f:
			...
			with open('spring_noise/params/spring_noise_' + str(std_dev_percent) + '.pickle', 'wb') as f2:
